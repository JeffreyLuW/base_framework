<template>
  <div class="app-container">
    ...
    <el-table border v-loading="loading" :data="typeList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="字典类型" align="center" :show-overflow-tooltip="true">
        <template slot-scope="scope">
          <el-button type="text" @click="goDictGroupEdit(scope.row)">{{ scope.row.dictType }}</el-button>
        </template>
      </el-table-column>
    ...
    <AutoDialog :title="title" :show.sync="open" width="500px">
      <el-form ref="form" :model="form" :rules="rules">
        <table class="theme-table-form">
          ...
          <tr>
            <td :class="themeTableLabelClass(rules.dictName)">字典名称</td>
          ...
          <tr>
            <td :class="themeTableLabelClass(rules.dictType)">字典类型</td>
            <td class="theme-table-form__input">
              <el-form-item label="字典类型" prop="dictType">
    ...
    <!--字典组dialog-->
    <AutoDialog sizeMode="fullscreen" width="1300px" :show.sync="showGroupDialog" title="字典项" :showFooter="false">
      <DictData v-if="showGroupDialog" :dictId="currentDictId"></DictData>
    </AutoDialog>
  </div>
</template>
...
<script>
  import {
    ...
    exportType,
    clearCache
  } from "@/platform/api/system/dict/type";
  ...
  export default {
    ...
    data() {
      let permMap = this.$perm.checkArrayToPermMap([
        ...
        {type: 'clearCache', permi: ['system:dict:remove']}
      ]);
      return {
        // 遮罩层
        loading: true,
        ...
        showGroupDialog: false,//编辑字典组的对话框
        currentDictId: null,
      };
    },
    created() {
    ...
    methods: {
      ...
      //点击编辑字典组
      goDictGroupEdit(row) {
        //:to="'/dict/type/data/' + scope.row.dictId" DictData
