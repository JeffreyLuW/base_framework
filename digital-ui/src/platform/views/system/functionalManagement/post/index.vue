    ...
    <AutoDialog :title="title" :show.sync="open" width="500px">
      <el-form ref="form" :model="form" :rules="rules">
        <table class="theme-table-form">
          ...
          <tr>
            <td :class="themeTableLabelClass(rules.postName)">岗位名称</td>
          ...
          <tr>
            <td :class="themeTableLabelClass(rules.postCode)">岗位编码</td>
            <td class="theme-table-form__input">
              <el-form-item label="岗位编码" prop="postCode">
          ...
          <tr>
            <td :class="themeTableLabelClass(rules.postSort)">岗位顺序</td>
            <td class="theme-table-form__input">
          ...
          <tr>
            <td :class="themeTableLabelClass(rules.status)">岗位状态</td>
            <td class="theme-table-form__input">
...
<script>
  import {listPost, getPost, delPost, addPost, updatePost, exportPost} from "@/platform/api/system/post";
  import {treeselect as menuTreeselect,postMenuTreeselect} from "@/platform/api/system/appMenu";
  ...
  export default {
    ...
    data() {
      let permMap = this.$perm.checkArrayToPermMap([
        {type: 'add', permi: ['system:post:add']},
        {type: 'edit', permi: ['system:post:edit']},
        {type: 'remove', permi: ['system:post:remove']},
        {type: 'export', permi: ['system:post:export']}
      ]);
      return {
        // 遮罩层
        loading: true,
    ...
    methods: {
      ...
      handleUpdate(row) {
        ...
        const postMenu = this.getPostMenuTreeselect(postId);
