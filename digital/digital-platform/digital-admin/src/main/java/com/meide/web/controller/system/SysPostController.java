...
public class SysPostController extends BaseController
{
    ...
    public AjaxResult add(@Validated @RequestBody SysPost post)
    {
        ...
        {
            return AjaxResult.errorWithMsg("新增岗位'" + post.getPostName() + "'失败，岗位名称已存在");
        }
        else if (UserConstants.NOT_UNIQUE.equals(postService.checkPostCodeUnique(post)))
        {
            return AjaxResult.errorWithMsg("新增岗位'" + post.getPostName() + "'失败，岗位编码已存在");
        }
        post.setCreateBy(SecurityUtils.getUsername());
        return rows(postService.insertPost(post));
    }
    ...
    public AjaxResult edit(@Validated @RequestBody SysPost post)
    {
        if (UserConstants.NOT_UNIQUE.equals(postService.checkPostNameUnique(post)))
        {
            return AjaxResult.errorWithMsg("修改岗位'" + post.getPostName() + "'失败，岗位名称已存在");
        }
        else if (UserConstants.NOT_UNIQUE.equals(postService.checkPostCodeUnique(post)))
        {
            return AjaxResult.errorWithMsg("修改岗位'" + post.getPostName() + "'失败，岗位编码已存在");
        }
        post.setUpdateBy(SecurityUtils.getUsername());
        return rows(postService.updatePost(post));
    }
    ...
    {
        return rows(postService.deletePostByIds(postIds));
    }
