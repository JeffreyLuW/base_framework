 ...
 * 更新非空值API
 */
public class UpdateSelectiveApi extends BaseApi {
    ...
    public String buildSql() {
        String sql = " <update id=\"updateSelective\" parameterType=\"map\">\n" +
                "    update {tableName} \n" +
                "    <set>\n" +
                "     {cols} " +
                "    </set>\n" +
        ...
        map.put("cols", this.cols.stream().map(c -> {
            return "<if test=\"" + c.getVoName() + " != null\">  " + c.getName() + " = #{" + c.getVoName() + JdbcTypeUtils.getTypeMapperDesc(c.getType()) + "}, </if>";
    ...
    public void registerToSwagger(Swagger swagger) {
        ...
        //添加接口定义
        SwaggerUtil.addPath_postjson(swagger, tag, this.tableInfo.getName() + "|修改非空值|" + EngineUtil.strVal(this.tableInfo.getComment()), this.getApiUrl(), this.tableInfo.getName() + "_" + getName() + "_param", null);
    }
}
